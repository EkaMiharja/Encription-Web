...
function setMode(mode) {
  ...
  fi.accept = mode === 'encrypt' ? '.txt,.pdf' : '.enc';

  const ivSection = document.getElementById('ivSection');
  const optionsRow = document.getElementById('optionsRow');

  if (mode === 'decrypt') {
    document.getElementById('dropSub').textContent = 'Pilih file terenkripsi:';
    document.getElementById('fileTypes').innerHTML = '<span class="file-type-tag" style="background:rgba(124,58,237,0.15);color:#a78bfa;border:1px solid rgba(124,58,237,0.3)">ENC</span>';
...
function handleFile(file) {
  ...
  if (currentMode === 'decrypt' && ext !== 'enc') {
    showAlert('error', '⚠ Hanya file .enc yang dapat didekripsi!'); return;
  }
  currentFile = file;
  clearAlert();
...
async function encryptFile(password) {
  ...
  downloadFile(payload, currentFile.name.replace(/\.[^.]+$/, '') + '.enc', 'text/plain');
